...
public class IdentityUtils {
    ...
    public static boolean isRealIDCard(String IdCard) {
        if (IdCard != null) {
            int correct = new IdentityUtils(IdCard).isCorrect();
            if (0 == correct) {// 符合规范
                return true;
            }
        }
    ...
    public static boolean isRealIDCardSec(String IdCard) {
        if (IdCard != null) {
            int[] ints = {7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2};
            int total = 0;
            for (int i = 0; i < ints.length; i++) {
                int plus = Integer.parseInt(String.valueOf(IdCard.charAt(i))) * ints[i];
                total += plus;
            }
            int mod = (12 - (total % 11)) % 11;
            System.out.println("参数total: " + total + "，参数mod: " + mod);
            if (mod != 10) {
                return String.valueOf(mod).equals(String.valueOf(IdCard.charAt(17)));
            } else {
                return (String.valueOf(IdCard.charAt(17)).equals("x") || String.valueOf(IdCard.charAt(17)).equals("X"));
            }
        }
        return false;
    }
     ...
     * @param idCardNum
     */
    public IdentityUtils(String idCardNum) {
        // super();
    ...
    public int isCorrect() {
        if (this.isEmpty()) {
            this.error = IdentityUtils.IS_EMPTY;
            return this.error;
        }

        if (!this.isLenCorrect()) {
            this.error = IdentityUtils.LEN_ERROR;
            return this.error;
        }

        if (!this.isCharCorrect()) {
            this.error = IdentityUtils.CHAR_ERROR;
            return this.error;
        }

        if (!this.isDateCorrect()) {
            this.error = IdentityUtils.DATE_ERROR;
            return this.error;
        }

        if (this.is18()) {
            if (!this.getCheck().equals(this.getCheckBit())) {
                this.error = IdentityUtils.CHECK_BIT_ERROR;
                return this.error;
            }
        }
