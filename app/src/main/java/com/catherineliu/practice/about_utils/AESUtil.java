...
public class AESUtil {
    ...
    private static byte[] encrypt(byte[] content, String password) {
        try {
            ...
            System.out.println(key.getFormat() + "key==" + key.getAlgorithm() + "====" + key.getEncoded());
            Cipher cipher = Cipher.getInstance(CipherMode);
            cipher.init(Cipher.ENCRYPT_MODE, key, new IvParameterSpec(
//                    "0000000000000000".getBytes()));
                    new byte[cipher.getBlockSize()]));
//            System.out.println(new byte[16] + "---cipher.getBlockSize()==" + cipher.getBlockSize());
...
//            data = hex2ByteArray(content);
        ...
        try {
            ...
            System.out.print("解密后: " + result);
//            MyLog.d("解密后", "result=" + result);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return result;
